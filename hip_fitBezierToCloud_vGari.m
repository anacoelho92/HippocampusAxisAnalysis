function [P,ctrl_pts] = hip_fitBezierToCloud_vGari(M,L,mydecimate,optim, subj_name)
%Document it. 
%  
%
...
if DEBUG
    ...
    hold on, plot3([Xa(1) Xp(1)],[Xa(2) Xp(2)],[Xa(3) Xp(3)],'r-','linewidth',3); %hold off
    hold on, plot3([Xa(1) Xc(1) Xp(1)],[Xa(2) Xc(2) Xp(2)],[Xa(3) Xc(3) Xp(3)],'k.','markersize',20); %hold off
...
pt1=[x_ini;y_ini;z_ini];
pt2=[x_int;y_int;z_int];
pt3=[x_end;y_end;z_end];
...
if DEBUG
    figure(1)
    hold on
    plot3(Xlist,Ylist,Zlist,'b','linewidth',3)
    plot3([x_ini x_int x_end],[y_ini y_int y_end],...
        [z_ini z_int z_end],'g.','markersize',20)
...
%savefig(['figures/subject_' subj_name,'.fig']);
hold off
